...
resource "local_file" "userdata_compiled" {
  ...
  filename = "${path.module}/temp/${var.main_project_tag}/userdata_compiled.sh"
}
...
resource "local_file" "vault_credentials" {
  ...
  filename = "${path.module}/temp/${var.main_project_tag}/vault_credentials.sh"
}
...
output "vpc_id" {
  value = data.aws_vpc.vault.id
}

output "vpc_ipv4_cidr" {
  value = data.aws_vpc.vault.cidr_block
}

output "vpc_ipv6_cidr" {
  value = data.aws_vpc.vault.ipv6_cidr_block
}